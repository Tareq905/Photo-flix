...
class MediaProcessor {
    ...
    static func exportVideo(url: URL, layer sketchLayer: CALayer, onSuccess: @escaping (URL) -> Void, onError: @escaping () -> Void) {
        DispatchQueue.global().async {
            ...
            Self.addSketchLayer(url: url, sketchLayer: sketchLayer) { result in
                ...
                let temporaryUrl = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("exported_video.mp4")
